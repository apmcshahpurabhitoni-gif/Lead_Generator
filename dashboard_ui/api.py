...
async def overview(r:Request):
    ...
    researched=sum(1 for x in leads if x.get("score") is not None or x.get("status") in {"RESEARCHED","QUALIFIED"})
...
async def discover(req:SearchRequest,r:Request):
    require_csrf(r); d=db(r); city=req.city.strip(); industry=req.category.strip()
    ...
    return {"ok":True,"job_id":job_id,"dataset_id":job_id,"status":"RUNNING","reused":False}
