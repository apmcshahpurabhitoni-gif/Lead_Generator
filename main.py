...
APP_VERSION = "webhook-self-heal-2"
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Never delete the Telegram webhook during shutdown.
        try:
...
async def unmatched_route(path: str, request: Request):
    """Return useful diagnostics instead of a generic 404."""
    ...
    return JSONResponse(
        ...
        content={
            ...
            "path": f"/{path}",
            "version": APP_VERSION,
            "routes": [p for p in route_paths() if p != "/{path:path}"],
        },
    )
