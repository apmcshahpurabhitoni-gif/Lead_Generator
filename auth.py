"""LeadHunter dashboard authentication and CSRF protection."""
...
def check_login_rate_limit(request):
    key=request.client.host if request.client else "unknown"; now=time.time(); q=_attempts[key]
    while q and now-q[0]>WINDOW:q.popleft()
    if len(q)>=MAX_ATTEMPTS: raise HTTPException(429,"Too many login attempts. Try again later.")
    q.append(now)
