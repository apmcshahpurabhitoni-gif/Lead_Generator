...
def menu(): return InlineKeyboardMarkup([[B("🏢 BUSINESSES","ui:biz:0")],[B("🔎 FIND LEADS","ui:biz:0")],[B("🔥 HOT LEADS","ui:hot:0")],[B("📋 OPEN LEAD","ui:open")],[B("💰 DEAL PIPELINE","ui:deal")],[B("📅 TODAY","ui:today"),B("📈 STATS","ui:stats")],[B("⏰ FOLLOW-UPS","ui:follow")],[B("❓ HELP","ui:help")]])
def biz_menu(page=0):
...
def city_menu(industry): return InlineKeyboardMarkup([[B("📍 JABALPUR",f"ui:search:Jabalpur:{industry}")],[B("✏️ OTHER CITY",f"ui:other:{industry}")],[B("⬅️ BUSINESSES","ui:biz:0"),B("🏠 MAIN MENU","ui:home")]])
def lead_menu(bid): return InlineKeyboardMarkup([[B("📋 AUDIT","ui:audit:%s"%bid),B("💬 MESSAGE","ui:message:%s"%bid)],[B("📞 CALL","ui:call:%s"%bid),B("⏰ FOLLOW-UP","ui:followlead:%s"%bid)],[B("💰 DEAL","ui:deallead:%s"%bid),B("📝 STATUS","ui:status:%s"%bid)],[B("🕘 HISTORY","ui:history:%s"%bid)],[B("🏠 MAIN MENU","ui:home")]])
def score_icon(s): s=int(s or 0); return "🔥" if s>=80 else "🟠" if s>=60 else "🟡" if s>=40 else "⚪"
...
async def show_hot(q,app,page=0):
    ...
    kb.append([B("🏢 BUSINESSES","ui:biz:0"),B("🏠 MENU","ui:home")])
...
async def callbacks(update,context):
    ...
    try:
        if action=="home": await q.edit_message_text("🚀 <b>LEADHUNTER</b>\n━━━━━━━━━━━━━━━━━━━━\n\n👇 Choose an action:",parse_mode="HTML",reply_markup=menu())
        ...
        elif action=="open": await q.edit_message_text("📋 <b>OPEN LEAD</b>\n\nUse <code>/lead LEAD_ID</code> or open one from 🔥 Hot Leads.",parse_mode="HTML",reply_markup=InlineKeyboardMarkup([[B("🔥 HOT LEADS","ui:hot:0")],[B("🏠 MAIN MENU","ui:home")]]))
            ...
            bid=int(parts[2]); lead=await db.get_lead(bid); await db.record_activity(bid,"CALL_COMPLETED","telegram","Manual call action recorded")
            await db.set_status(bid,"CONTACTED"); await q.edit_message_text("📞 <b>CALL RECORDED</b>\n━━━━━━━━━━━━━━━━━━━━\n\nPhone: <b>"+html.escape(str((lead or {}).get("phone") or "Not found"))+"</b>\n\n📝 Status updated to <b>CONTACTED</b>.",parse_mode="HTML",reply_markup=lead_menu(bid))
