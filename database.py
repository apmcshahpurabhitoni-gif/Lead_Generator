...
class Database:
    ...
    async def upsert_business(self,business:dict[str,Any])->tuple[int|None,bool]:
        identity=business.get("identity_key") or self.identity_key(business.get("name", ""),business.get("city", ""),business.get("website")); old=self.client.table("businesses").select("id").eq("identity_key",identity).limit(1).execute(); created=not bool(old.data)
        row={k:business.get(k) for k in ["name","industry","city","website","phone","email","source","source_attribution","source_place_id"]}; row["identity_key"]=identity; result=self.client.table("businesses").upsert(row,on_conflict="identity_key").execute()
    ...
    async def save_research_and_score(self,bid:int,research:dict[str,Any],score:dict[str,Any])->None:
        self.client.table("research").insert({"business_id":bid,"research_json":research,"problems":research.get("problems",[])}).execute(); self.client.table("businesses").update({"score":score["score"],"priority":score["priority"],"recommended_services":score["recommended_services"],"problems":score["reasons"],"status":"QUALIFIED" if score["score"]>=60 else "RESEARCHED","updated_at":datetime.now(timezone.utc).isoformat()}).eq("id",bid).execute(); await self.record_activity(bid,"RESEARCHED","system",f"Score={score['score']}")
    ...
    async def list_leads(self,priority:str|None=None,limit:int=10,offset:int=0)->list[dict[str,Any]]:
        limit=max(1,min(limit,1000)); q=self.client.table("businesses").select("*").order("score",desc=True).range(offset,offset+limit-1); q=q.eq("priority",priority) if priority else q; r=q.execute(); return r.data or []
    async def list_search_results(self,city:str|None,industry:str|None,limit:int=8,offset:int=0)->list[dict[str,Any]]:
        ...
        limit=max(1,min(limit,1000)); q=self.client.table("businesses").select("*").eq("city",city).eq("industry",industry).order("score",desc=True).range(offset,offset+limit-1); rows=q.execute().data or []
        for row in rows: row["research"]=await self.get_research(int(row["id"]))
        return rows
    ...
    async def record_activity(self,bid:int,action:str,channel:str,notes:str="")->None:
        self.client.table("activities").insert({"business_id":bid,"action":action,"channel":channel,"notes":notes[:2000]}).execute(); mapping={"CALL_COMPLETED":{"calls":1},"STATUS_CONTACTED":{"contacted":1},"STATUS_RESPONDED":{"replies":1},"STATUS_MEETING":{"meetings":1},"STATUS_PROPOSAL":{"proposals":1},"STATUS_WON":{"won":1},"STATUS_LOST":{"lost":1},"DEAL_PROPOSAL":{"proposals":1},"DEAL_WON":{"won":1},"DEAL_LOST":{"lost":1}}; 
    ...
    async def due_followups(self,limit:int=10)->list[dict[str,Any]]:
        now=datetime.now(timezone.utc).isoformat(); r=self.client.table("followups").select("id,business_id,due_at,notes,businesses(name)").eq("status","OPEN").lte("due_at",now).order("due_at").limit(min(limit,20)).execute(); return [{"id":x["id"],"business_id":x["business_id"],"business_name":(x.get("businesses") or {}).get("name","Unknown"),"due_at":x["due_at"],"notes":x.get("notes") or ""} for x in (r.data or [])]
    ...
    async def list_deals(self,limit:int=20)->list[dict[str,Any]]:
        r=self.client.table("deals").select("id,business_id,value,services,stage,notes,created_at,businesses(name)").order("updated_at",desc=True).limit(min(limit,50)).execute(); return [{"id":x["id"],"business_id":x["business_id"],"business_name":(x.get("businesses") or {}).get("name","Unknown"),"value":x.get("value"),"services":x.get("services") or [],"stage":x.get("stage"),"notes":x.get("notes") or ""} for x in (r.data or [])]
