...
async def research_business(business: dict[str,Any])->dict[str,Any]:
    ...
    result={"website":{"exists":bool(website),"pages":[],"errors":[],"robots_checked":False},"seo":{},"local":{},"search":{"status":"NOT_CONFIGURED"},"technology":{"signals":[]},"buying_signals":[],"problems":[]}
    if not website:
    ...
    if not await robots_allowed(website,website+"/"):
        result["website"]["robots_checked"]=True; result["website"]["errors"].append("Website could not be crawled because robots.txt was unavailable or did not permit LeadHunter."); result["problems"].append("Website could not be crawled under the site's robots policy."); return result
