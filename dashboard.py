...
async def dashboard(authorization: str | None = Header(default=None)):
    ...
    cards_html = "".join(
        f'<div class="stat-card"><span>{icon}</span><div><small>{html.escape(label)}</small><strong>{value}</strong></div></div>'
        for icon, label, value in cards
    )
    rows = "".join(
        f'<tr><td>{html.escape(str(r.get("date", "")))}</td><td>{r.get("leads_found", 0)}</td><td>{r.get("contacted", 0)}</td><td>{r.get("replies", 0)}</td><td>{r.get("meetings", 0)}</td><td>{r.get("won", 0)}</td></tr>'
        for r in history
    )
    return HTMLResponse(
        f'''<!doctype html>
...
<aside class="sidebar">
  ...
  <nav class="nav">
    ...
    <button type="button" onclick="document.getElementById('leadWorkspace').scrollIntoView({{behavior:'smooth'}})">👥 Leads</button>
    <button type="button" onclick="focusSearch()">🔎 Find / Filter</button>
    <button type="button" onclick="document.getElementById('history').scrollIntoView({{behavior:'smooth'}})">📊 Analytics</button>
    <button type="button">✉️ Outreach</button>
    <button type="button">⚙️ Settings</button>
  </nav>
</aside>
<main class="main">
  <header class="topbar">
    <div><div class="eyebrow">Lead workspace</div><h1>Business leads</h1><div class="muted">Persistent records · dynamic locations · manual research shortcuts</div></div>
    <div class="top-actions"><button class="btn" onclick="loadLeads()">↻ Refresh</button><button class="btn primary" onclick="focusSearch()">＋ Search leads</button></div>
  </header>
  <section class="stats">{cards_html}</section>
  <section class="workspace" id="leadWorkspace">
    <aside class="panel categories" id="categories"><div class="nav-label" style="padding-top:4px">Business types</div><div class="empty"><span class="spinner"></span></div></aside>
    <section class="panel content">
      <div class="content-head">
        <div class="content-head-row"><div><div class="eyebrow" id="categoryEyebrow">All businesses</div><h2 id="categoryTitle">All leads</h2></div><span class="muted" id="leadCount"></span></div>
        <div class="search"><input id="leadSearch" type="search" autocomplete="off" placeholder="Search business, city, phone or website…" oninput="render()"><button class="btn" onclick="clearSearch()">Clear</button></div>
      </div>
      <div class="locations" id="locations"></div>
      <div class="leads" id="leads"><div class="empty"><span class="spinner"></span><strong>Loading saved leads…</strong></div></div>
    </section>
  </section>
...
<script>
const stages=[['NEW','🔎 Discovered'],['VERIFIED','🧹 Verified'],['QUALIFIED','🎯 Qualified'],['RESEARCHED','📋 Researched'],['MESSAGE_GENERATED','✉️ Message generated'],['CONTACTED','📤 Contacted'],['RESPONDED','💬 Replied'],['MEETING','📅 Meeting'],['PROPOSAL','📄 Proposal'],['WON','💰 Won'],['LOST','❌ Lost']];
...
async function loadLeads(){{
  document.getElementById('leads').innerHTML='<div class="empty"><span class="spinner"></span><strong>Loading saved leads…</strong></div>';
  try{{const r=await fetch('/dashboard/api/leads?limit=500');if(!r.ok)throw new Error('Failed to load leads');leads=await r.json();buildNavigation();render();toast('Lead database refreshed')}}catch(e){{document.getElementById('leads').innerHTML='<div class="empty"><strong>Could not load leads</strong><div>'+esc(e.message)+'</div></div>'}}
}}
function buildNavigation(){{
  const groups={{}};leads.forEach(l=>{{const k=(l.industry||'Other').trim()||'Other';(groups[k]??=[]).push(l)}});
  const cats=document.getElementById('categories');cats.innerHTML='<div class="nav-label" style="padding-top:4px">Business types</div>';
  const allBtn=document.createElement('button');allBtn.className='category '+(selectedCategory==='ALL'?'active':'');allBtn.innerHTML='<span>✨ All businesses</span><span class="count">'+leads.length+'</span>';allBtn.onclick=()=>{{selectedCategory='ALL';selectedCity='ALL';buildNavigation();render()}};cats.appendChild(allBtn);
  Object.entries(groups).sort((a,b)=>a[0].localeCompare(b[0])).forEach(([cat,items])=>{{const b=document.createElement('button');b.className='category '+(selectedCategory===cat?'active':'');b.innerHTML='<span>'+esc(cat)+'</span><span class="count">'+items.length+'</span>';b.onclick=()=>{{selectedCategory=cat;selectedCity='ALL';buildNavigation();render()}};cats.appendChild(b)}});
  const locs=leads.filter(l=>selectedCategory==='ALL'||norm(l.industry)===norm(selectedCategory)).reduce((m,l)=>{{const c=(l.city||'Unknown').trim()||'Unknown';m[c]=(m[c]||0)+1;return m}},{{}});
  const locEl=document.getElementById('locations');locEl.innerHTML='';
  const all=document.createElement('button');all.className='tab '+(selectedCity==='ALL'?'active':'');all.textContent='ALL '+Object.values(locs).reduce((a,b)=>a+b,0);all.onclick=()=>{{selectedCity='ALL';render()}};locEl.appendChild(all);
  Object.entries(locs).sort((a,b)=>a[0].localeCompare(b[0])).forEach(([city,count])=>{{const t=document.createElement('button');t.className='tab '+(selectedCity===city?'active':'');t.textContent=city+' '+count;t.onclick=()=>{{selectedCity=city;render()}};locEl.appendChild(t)}});
}}
...
function render(){{
 const list=filtered();document.getElementById('categoryTitle').textContent=selectedCategory==='ALL'?'All leads':selectedCategory;document.getElementById('categoryEyebrow').textContent=selectedCity==='ALL'?'All locations':selectedCity;document.getElementById('leadCount').textContent=list.length+' saved lead'+(list.length===1?'':'s');
 const el=document.getElementById('leads');if(!list.length){{el.innerHTML='<div class="empty"><strong>No leads in this view</strong><div>Run a search or choose another business type/location.</div></div>';return}}
 el.innerHTML=list.map((l,i)=>leadCard(l,i)).join('');
}}
function leadCard(l,i){{
 const open=openLead===l.id?' open':'';const score=Number(l.score||0);const tone=score>=85?'hot':score>=60?'warm':'';const status=stages.find(s=>s[0]===l.status)?.[1]||l.status||'🔎 Discovered';
 const services=(l.recommended_services||[]).slice(0,8).map(x=>'<span class="opp">'+esc(x)+'</span>').join('');
 const links=[l.website?'<a class="link" target="_blank" rel="noopener noreferrer" href="'+esc(l.website)+'">🌐 Website</a>':'', '<a class="link" target="_blank" rel="noopener noreferrer" href="'+mapsUrl(l)+'">📍 Maps</a>'].filter(Boolean).join('');
 const checks=stages.map(([key,label])=>'<label class="status-check '+(l.status===key?'active':'')+'"><input type="radio" name="status-'+l.id+'" '+(l.status===key?'checked':'')+' onchange="setStatus('+l.id+',\\''+key+'\\')">'+label+'</label>').join('');
 const activity=l.updated_at?'Last updated · '+new Date(l.updated_at).toLocaleString():'';
 return '<article class="lead'+open+'" data-id="'+l.id+'">'+
 '<div class="lead-summary" onclick="toggleLead('+l.id+')"><div class="lead-name">'+(l.industry?'🏢 ':'')+esc(l.name)+'</div><div class="lead-meta">📍 '+esc(l.city||'—')+'</div><div class="rating">⭐ '+esc(l.rating??'—')+'</div><div class="score '+tone+'">'+score+'/100</div><span class="status-pill">'+esc(status)+'</span><button class="google" type="button" onclick="event.stopPropagation();window.open(googleUrl('+JSON.stringify(l)+'),\\'_blank\\',\\'noopener,noreferrer\\')">🔎 Google</button><span class="chevron">⌄</span></div>'+
 '<div class="details"><div class="detail-grid">'+
 '<div class="section"><h3>Business information</h3><div class="facts"><b>Business</b><span>'+esc(l.name)+'</span><b>Category</b><span>'+esc(l.industry||'—')+'</span><b>Location</b><span>'+esc(l.city||'—')+'</span><b>Phone</b><span>'+esc(l.phone||'—')+'</span><b>Email</b><span>'+esc(l.email||'—')+'</span><b>Website</b><span>'+esc(l.website||'—')+'</span><b>Lead score</b><span><strong>'+score+'/100</strong> · '+esc(l.priority||'LOW')+'</span></div></div>'+ 
 '<div class="section"><h3>Quick research</h3><div class="links">'+links+'<a class="link" target="_blank" rel="noopener noreferrer" href="'+googleUrl(l)+'">🔎 Search business</a></div><div class="opps" style="margin-top:10px">'+(services||'<span class="muted">No recommended services yet</span>')+'</div></div>'+ 
 '<div class="section"><h3>Lead stage</h3><div class="status-grid">'+checks+'</div></div>'+ 
 '<div class="section"><h3>Notes & activity</h3><div class="muted">'+esc((l.problems||[]).slice(0,5).join(' · ')||'No problems recorded.')+'</div><div class="activity">'+esc(activity)+'</div><div class="detail-actions"><button class="btn" onclick="event.stopPropagation();window.open(googleUrl('+JSON.stringify(l)+'),\\'_blank\\',\\'noopener,noreferrer\\')">🔎 Research on Google</button></div></div>'+ 
 '</div></div></article>'}}
...
async def dashboard_set_status(
    ...
    require_dashboard(authorization)
    allowed = {key for key, _ in [
        ("NEW", "Discovered"), ("VERIFIED", "Verified"), ("QUALIFIED", "Qualified"),
        ("RESEARCHED", "Researched"), ("MESSAGE_GENERATED", "Message generated"),
        ("CONTACTED", "Contacted"), ("RESPONDED", "Replied"), ("MEETING", "Meeting"),
