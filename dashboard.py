...
def _authorized(token: str | None) -> bool:
    ...
    # Fail closed: dashboard access requires an explicit token.
...
async def home(x_dashboard_token: str | None = Header(default=None)):
    ...
    history = await db.history(days=14)
    rows = "".join(
        f"<tr><td>{r.get('date','')}</td><td>{r.get('leads_found',0)}</td>"
        f"<td>{r.get('contacted',0)}</td><td>{r.get('replies',0)}</td>"
        f"<td>{r.get('meetings',0)}</td><td>{r.get('won',0)}</td></tr>"
        for r in history
    )
...
.card{{background:white;border-radius:14px;padding:18px;box-shadow:0 2px 12px rgba(0,0,0,.06)}}
...
th,td{{padding:12px;border-bottom:1px solid #eef0f4;text-align:left}}th{{font-size:13px;color:#687386}}
</style></head><body>
...
<table><thead><tr><th>Date</th><th>Leads</th><th>Contacted</th><th>Replies</th><th>Meetings</th><th>Won</th></tr></thead>
<tbody>{rows or '<tr><td colspan="6">No history yet.</td></tr>'}</tbody></table>
<p class="muted">Telegram is the primary operating interface. This dashboard is intentionally minimal.</p>
</body></html>
""")
