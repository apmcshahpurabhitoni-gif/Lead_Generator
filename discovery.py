...
POLICIES = {
    "nominatim": {"rpm": 50, "daily": 500, "delay": 1.1, "retries": 2},
    "overpass": {"rpm": 6, "daily": 100, "delay": 10.0, "retries": 2},
    "website": {"rpm": 12, "daily": 500, "delay": 5.0, "retries": 2},
}
_usage: dict[str, deque[float]] = defaultdict(deque)
_daily: dict[str, tuple[str, int]] = {}
_locks: dict[str, asyncio.Lock] = defaultdict(asyncio.Lock)
INDUSTRY_TAGS = {
    "dental": [("amenity", "dentist")], "dentist": [("amenity", "dentist")], "hospital": [("amenity", "hospital")],
    "clinic": [("amenity", "clinic")], "restaurant": [("amenity", "restaurant")], "cafe": [("amenity", "cafe")],
    "bakery": [("shop", "bakery")], "hotel": [("tourism", "hotel")], "resort": [("tourism", "resort")],
    "school": [("amenity", "school")], "college": [("amenity", "college")], "university": [("amenity", "university")],
    "pharmacy": [("amenity", "pharmacy")], "gym": [("leisure", "fitness_centre")], "salon": [("shop", "hairdresser")],
    "beauty": [("shop", "beauty")], "car dealer": [("shop", "car")], "car repair": [("shop", "car_repair")],
    "real estate": [("office", "estate_agent")], "lawyer": [("office", "lawyer")], "accountant": [("office", "accountant")],
    "travel agency": [("shop", "travel_agency")], "electronics": [("shop", "electronics")], "clothing": [("shop", "clothes")],
    "furniture": [("shop", "furniture")], "jewellery": [("shop", "jewelry")], "jewelry": [("shop", "jewelry")],
    "supermarket": [("shop", "supermarket")], "hardware": [("shop", "hardware")], "bank": [("amenity", "bank")],
    "insurance": [("office", "insurance")], "architect": [("office", "architect")], "printing": [("shop", "printing")],
    "photographer": [("shop", "photo")], "car wash": [("amenity", "car_wash")], "fuel": [("amenity", "fuel")],
...
async def _budget(source: str) -> None:
    ...
    async with _locks[source]:
        ...
        if q and len(q) >= p["rpm"]: await asyncio.sleep(max(0.5, 60 - (now - q[0])))
        if q:
...
async def request(source: str, method: str, url: str, **kwargs: Any) -> httpx.Response:
    await _budget(source); p = POLICIES[source]; last: Exception | None = None
...
def _overpass_filters(industry: str) -> str:
    tags = INDUSTRY_TAGS.get(industry.strip().lower())
    if tags:
        return "".join(f'node["{k}"="{v}"](area.searchArea);way["{k}"="{v}"](area.searchArea);relation["{k}"="{v}"](area.searchArea);' for k, v in tags)
    safe = "".join(c for c in industry.strip().lower() if c.isalnum() or c in " _-")[:50]
...
async def discover_businesses(city: str, industry: str, limit: int = 50) -> list[dict[str, Any]]:
    ...
    elements = response.json().get("elements", [])[:max(1, min(limit, 50))]
    results, seen = [], set()
    for item in elements:
        ...
        seen.add(identity)
        results.append({
            "name": name, "industry": industry, "city": city, "website": normalize_website(website),
            "phone": tags.get("phone") or tags.get("contact:phone"), "email": tags.get("email") or tags.get("contact:email"),
