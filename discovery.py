...
# Public OSM documents currently list these as global-coverage Overpass instances.
OVERPASS_URLS=[
    "https://overpass.private.coffee/api/interpreter",
    "https://overpass-api.de/api/interpreter",
    "https://maps.mail.ru/osm/tools/overpass/api/interpreter",
]
USER_AGENT="LeadHunter/2.0 (business research; OSM discovery)"
POLICIES={"nominatim":{"rpm":50,"daily":500,"delay":1.1,"retries":0},"overpass":{"rpm":6,"daily":100,"delay":10.0,"retries":0},"website":{"rpm":12,"daily":500,"delay":1.0,"retries":1}}
...
INDUSTRY_TAGS={
"dental":[("amenity","dentist")],"dentist":[("amenity","dentist")],"hospital":[("amenity","hospital")],"clinic":[("amenity","clinic")],"restaurant":[("amenity","restaurant")],"cafe":[("amenity","cafe")],"bakery":[("shop","bakery")],"hotel":[("tourism","hotel")],"resort":[("tourism","resort")],"school":[("amenity","school")],"college":[("amenity","college")],"university":[("amenity","university")],"pharmacy":[("amenity","pharmacy")],"gym":[("leisure","fitness_centre")],"salon":[("shop","hairdresser")],"beauty":[("shop","beauty")],"car dealer":[("shop","car")],"car repair":[("shop","car_repair")],"car wash":[("amenity","car_wash")],"real estate":[("office","estate_agent")],"lawyer":[("office","lawyer")],"accountant":[("office","accountant")],"travel agency":[("shop","travel_agency")],"electronics":[("shop","electronics")],"clothing":[("shop","clothes")],"furniture":[("shop","furniture")],"jewellery":[("shop","jewelry")],"jewelry":[("shop","jewelry")],"supermarket":[("shop","supermarket")],"hardware":[("shop","hardware")],"bank":[("amenity","bank")],"insurance":[("office","insurance")],"architect":[("office","architect")],"construction":[("office","construction_company")],"printing":[("shop","printing")],"photographer":[("shop","photo")],"fuel":[("amenity","fuel")],"veterinary":[("amenity","veterinary")],
}
...
def _timeout(seconds): return httpx.Timeout(seconds,connect=min(10.0,seconds))
...
async def request(source:str,method:str,url:str,*,timeout:float=20,**kwargs:Any)->httpx.Response:
    ...
    await _budget(source); headers={"User-Agent":USER_AGENT,**(kwargs.pop("headers",{}) or {})}
    ...
    for attempt in range(POLICIES[source]["retries"]+1):
        try:
            async with httpx.AsyncClient(follow_redirects=True,headers=headers,timeout=_timeout(timeout)) as client:
                response=await client.request(method,url,**kwargs); response.raise_for_status(); return response
        except Exception as exc:
            last=exc; log.warning("%s request failed | attempt=%s | %s | %s",source,attempt+1,url,exc)
            if attempt<POLICIES[source]["retries"]: await asyncio.sleep(min(8,2**attempt))
...
async def _city_area(city:str):
    ...
    else: raise RuntimeError(f"City '{city}' did not resolve to an OSM area")
...
async def _overpass(query:str):
    ...
    for url in OVERPASS_URLS:
        try:
            # One attempt per mirror; move immediately to another mirror on timeout/5xx/TLS errors.
...
async def discover_businesses(city:str,industry:str,limit:int=50)->list[dict[str,Any]]:
    ...
    if payload.get("remark") and not payload.get("elements"): raise RuntimeError("Overpass rejected or terminated the discovery query: "+str(payload["remark"])[:500])
    elements=payload.get("elements",[])[:safe_limit]; results=[]; seen=set()
