...
INDUSTRY_TAGS = {
    ...
    "real estate": [("office", "estate_agent")], "lawyer": [("office", "lawyer")],
    "accountant": [("office", "accountant")], "travel agency": [("shop", "travel_agency")],
    "electronics": [("shop", "electronics")], "clothing": [("shop", "clothes")],
    "furniture": [("shop", "furniture")], "jewellery": [("shop", "jewelry")],
    "jewelry": [("shop", "jewelry")], "supermarket": [("shop", "supermarket")],
    "hardware": [("shop", "hardware")], "bank": [("amenity", "bank")],
    "insurance": [("office", "insurance")], "architect": [("office", "architect")],
    "printing": [("shop", "printing")], "photographer": [("shop", "photo")],
    "car wash": [("amenity", "car_wash")], "fuel": [("amenity", "fuel")],
...
ALL_TAGS = [
    ...
    ("office", x) for x in ["estate_agent", "lawyer", "accountant", "insurance", "architect"]
]
...
async def _request_once(source: str, method: str, url: str, *, timeout: float, **kwargs: Any) -> httpx.Response:
...
async def _request_with_retries(source: str, method: str, url: str, *, timeout: float, **kwargs: Any) -> httpx.Response:
    ...
    for attempt in range(retries + 1):
        try:
            ...
            last = exc
            log.warning("%s endpoint failed: %s attempt=%s error=%s", source, url, attempt + 1, exc)
            if attempt < retries:
...
async def _overpass_request(query: str) -> httpx.Response:
    ...
    for url in OVERPASS_URLS:
        try:
            return await _request_with_retries("overpass", "POST", url, data=query, timeout=35)
        except Exception as exc:
            errors.append(f"{url}: {type(exc).__name__}: {exc}")
    detail = " | ".join(errors)[-1600:]
    raise RuntimeError(
        "All discovery connections failed. Overpass providers were unavailable. "
        "Please retry in a moment. Details: " + detail
    )
...
async def _city_area(city: str) -> tuple[int, str]:
    ...
    response = await _request_with_retries(
        "nominatim", "GET", NOMINATIM_URL,
        params={"q": f"{city}, India", "format": "jsonv2", "limit": 1, "countrycodes": "in"},
        timeout=15,
        referer=os.environ.get("WEBHOOK_BASE_URL"),
    )
    rows = response.json()
    if not rows:
    ...
    if osm_type == "relation":
        ...
        raise RuntimeError(f"City '{city}' resolved to a point instead of a boundary. Try another city spelling.")
...
async def discover_businesses(city: str, industry: str, limit: int = 50) -> list[dict[str, Any]]:
    ...
    elements = response.json().get("elements", [])[:safe_limit]
    ...
    log.info("Discovery complete | city=%s | industry=%s | results=%s", city, industry, len(results))
    return results
