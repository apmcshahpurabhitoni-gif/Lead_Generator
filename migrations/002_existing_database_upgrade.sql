-- LeadHunter v3.3.0 existing-database upgrade. Safe to run repeatedly.
...
create table if not exists search_results (search_id bigint not null references jobs(id) on delete cascade, business_id bigint not null references businesses(id) on delete cascade, result_rank integer, created_at timestamptz default now(), primary key(search_id,business_id));
