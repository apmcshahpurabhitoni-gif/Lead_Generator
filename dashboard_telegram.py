...
async def send_lead_to_telegram(business_id:int,request:Request,authorization:str|None=Header(default=None)):
    ...
    phone=lead.get('phone') or ((loc.get('phones') or [None])[0] if isinstance(loc.get('phones'),list) else None); email=lead.get('email') or ((loc.get('emails') or [None])[0] if isinstance(loc.get('emails'),list) else None)
